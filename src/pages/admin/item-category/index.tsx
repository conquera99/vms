...
const Page = () => {
	...
	return (
        <Navigation title="VMS: Data Kategori" active="admin" access="item_category" isAdmin>
            <Container>
				<div className="mt-6 rounded-3xl border border-slate-200 bg-linear-to-br from-emerald-50 via-white to-cyan-50 p-5 shadow-sm md:p-6">
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							<Breadcrumb data={breadcrumb} />
							<h1 className="mt-3 text-2xl font-bold text-slate-800">Data Kategori Item</h1>
							<p className="text-sm text-slate-600">Atur kategori agar manajemen item lebih rapi.</p>
						</div>
						<div className="flex w-full flex-col items-end gap-2 md:w-auto">
							<div className="min-w-36 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-emerald-900 shadow-sm">
								<div className="flex items-center justify-between gap-3">
									<div>
										<p className="text-[10px] uppercase tracking-[0.18em] text-emerald-600">Total Data</p>
										<p className="mt-1 text-2xl font-black leading-none">{shownCount}</p>
									</div>
									<TagOutline className="text-xl text-emerald-700" />
								</div>
							</div>
