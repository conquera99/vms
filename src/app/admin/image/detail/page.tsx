...
import { CloseOutline, FolderOutline, PicturesOutline, RightOutline } from 'components/general/antd-icon';
...
const Page = () => {
	...
	useEffect(() => {
		...
		if (query.get('id')) {
			axios.get(`/api/admin/gallery/image?id=${query.get('id')}`).then((response) => {
				if (response.data.code === 0) {
							...
							<div className="mt-6 border-t border-slate-100 pt-6">
								...
								<div className="mt-4">
									<Upload
										file={file}
										image={image}
										disabled={!query.get('id') ? false : true}
