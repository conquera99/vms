...
import { CloseOutline, GlobalOutline, RightOutline, TagOutline } from 'components/general/antd-icon';
...
const Page = () => {
	...
	useEffect(() => {
		if (query.get('id')) {
			axios.get(`/api/admin/permission?id=${query.get('id')}`).then((response) => {
				if (response.data.code === 0) {
