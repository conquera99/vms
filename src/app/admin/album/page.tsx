...
import { AddOutline, FolderOutline, LeftOutline } from 'components/general/antd-icon';
			...
			<Container>
					...
					<div className="flex flex-col gap-6 p-6 md:p-7 lg:flex-row lg:items-end lg:justify-between">
						...
						<Breadcrumb data={breadcrumb} />
							...
							<h1 className="mt-4 text-3xl font-black tracking-tight text-slate-800 sm:text-4xl">
								Koleksi Album Galeri
							</h1>
							<p className="mt-3 max-w-xl text-sm leading-relaxed text-slate-600 sm:text-base">
								Susun album galeri agar pengelompokan dokumentasi kegiatan lebih rapi dan mudah
								ditelusuri.
							</p>
						</div>
				...
				{isEmpty && (
					<Empty
						...
						desc="Tambahkan album baru untuk mulai mengelompokkan dokumentasi galeri."
						action={
							<LinkButton
								href="/admin/album/detail"
								size="small"
								...
								<div className="flex flex-col gap-5">
									<div className="flex items-start justify-between gap-4">
										...
										<div className="rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-3 text-right">
											...
											<p className="mt-1 text-sm font-bold text-slate-800">Siap diisi gambar</p>
										</div>
									</div>
									...
									<div className="grid grid-cols-2 gap-3">
										<div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
											...
											<p className="mt-2 text-sm font-semibold text-slate-700">Mengelompokkan dokumentasi</p>
										</div>
										<div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
											<p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">
												Aksi
											</p>
											<p className="mt-2 text-sm font-semibold text-slate-700">Edit metadata album</p>
										</div>
									</div>

									<div className="flex items-center justify-end gap-3 border-t border-slate-100 pt-4">
										<Link
											href={`/admin/album/detail?id=${item.id}`}
