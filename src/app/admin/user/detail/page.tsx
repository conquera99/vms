...
import { CloseOutline, RightOutline, TagOutline, UserContactOutline } from 'components/general/antd-icon';
...
const Page = () => {
	...
	useEffect(() => {
		if (query.get('id')) {
			...
			axios.get(`/api/admin/user?id=${query.get('id')}`).then((response) => {
				if (response.data.code === 0) {
						...
						>
							...
							<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
								...
								<Input
									name="password"
									label="Password"
									required={!query.get('id')}
									type="password"
									className="mb-0 md:col-span-2"
									rules={
										!query.get('id') && [{ required: true, message: 'password wajib diisi' }]
									}
								/>
								{query.get('id') && (
									<Select
