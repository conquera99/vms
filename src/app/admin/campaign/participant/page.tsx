...
import { CloseOutline, RightOutline, SendOutline } from 'components/general/antd-icon';
...
const Page = () => {
	...
	const onRemove = (seq: number) => {
		...
		axios
			.post('/api/admin/campaign/participant/remove', { id: query.get('id') as string, seq })
			.then((response) => {
				if (response.data.code === 0) {
					toast.success(successMessage);
					loadParticipant();
	...
	const loadParticipant = useCallback(() => {
		axios.get(`/api/admin/campaign/participant?id=${query.get('id')}`).then((response) => {
			if (response.data.code === 0) {
	...
	useEffect(() => {
		if (query.get('id')) {
			axios.get(`/api/admin/campaign?id=${query.get('id')}`).then((response) => {
				if (response.data.code === 0) {
