...
const Page = ({ params }: { params: { id: string } }) => {
	...
	useEffect(() => {
		axios.get(`/api/admin/permission?id=${params.id}`).then((response) => {
			if (response.data.code === 0) {
	...
	return (
		<Navigation title="VMS: Hak Akses Detail" active="admin" access="permission" isAdmin>
			<ContainerAdmin>
					...
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<p className="text-sm text-slate-600">
								Kelola data hak akses dan izin user vihara.
							</p>
						</div>
						<LinkButton
							href="/admin/permission"
					...
					<Form layout="vertical" form={form} onFinish={onFinish} initialValues={{ name: '' }}>
						<div className="grid grid-cols-1 gap-3">
							<Input
								...
								required
								className="mb-0"
								rules={[{ required: true, message: 'nama hak akses wajib diisi' }]}
							/>
						</div>
