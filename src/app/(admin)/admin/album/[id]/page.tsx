...
const Page = (props: { params: Promise<{ id: string }> }) => {
    ...
    useEffect(() => {
		axios.get(`/api/admin/album?id=${params.id}`).then((response) => {
			if (response.data.code === 0) {
					...
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<p className="text-sm text-slate-600">
								Kelola data album galeri foto vihara.
							</p>
						</div>
						<LinkButton
							href="/admin/album"
					...
					<Form layout="vertical" form={form} onFinish={onFinish} initialValues={{ name: '' }}>
						<div className="grid grid-cols-1 gap-3">
							<Input
								...
								required
								className="mb-0"
								rules={[{ required: true, message: 'nama album wajib diisi' }]}
							/>
						</div>
