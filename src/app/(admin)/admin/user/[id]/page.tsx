...
const Page = (props: { params: Promise<{ id: string }> }) => {
    ...
    useEffect(() => {
		axios.get(`/api/admin/user?id=${params.id}`).then((response) => {
			if (response.data.code === 0) {
    ...
    return (
		<Navigation title="VMS: User Detail" active="admin" access="user" isAdmin>
			<ContainerAdmin>
					...
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<p className="text-sm text-slate-600">
								Kelola data user dan hak akses vihara.
							</p>
						</div>
						<LinkButton
							href="/admin/user"
