...
const Page = ({ params }: { params: { id: string } }) => {
	...
	useEffect(() => {
		axios.get(`/api/admin/item-category?id=${params.id}`).then((response) => {
			if (response.data.code === 0) {
	...
	return (
		<Navigation title="VMS: Kategori Item Detail" active="admin" access="item_category" isAdmin>
			<ContainerAdmin>
					...
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<p className="text-sm text-slate-600">
								Kelola data kategori untuk pengelompokan item vihara.
							</p>
						</div>
						<LinkButton
							href="/admin/item-category"
