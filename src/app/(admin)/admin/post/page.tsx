...
const Page = () => {
	...
	const onRemove = (id: string) => {
		axios.post('/api/admin/post/remove', { id }).then((response) => {
			...
			<Container>
				<div className="rounded-3xl border border-slate-200 bg-linear-to-br from-cyan-50 via-white to-amber-50 p-5 shadow-sm md:p-6">
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<h1 className="mt-3 text-2xl font-bold text-slate-800">Data Post</h1>
							<p className="text-sm text-slate-600">Kelola artikel dan postingan vihara.</p>
						</div>
						<div className="flex w-full flex-col items-end gap-2 md:w-auto">
							<div className="min-w-36 rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-2 text-cyan-900 shadow-sm">
								<div className="flex items-center justify-between gap-3">
									...
									<ContentOutline className="text-xl text-cyan-700" />
								</div>
							</div>
				...
				{isEmpty && (
					<Empty
						...
						desc="Tambahkan post baru untuk artikel dan berita vihara."
						action={
							<LinkButton
								href="/admin/post/create"
								size="small"
