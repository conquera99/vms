...
const Page = () => {
	...
	const onRemove = (id: string) => {
		axios.post('/admin/assign-item/remove', { id }).then((response) => {
	...
	return (
		<Navigation title="VMS: Data Atur Lokasi" active="admin" access="location" isAdmin>
			<Container>
				<div className="rounded-3xl border border-slate-200 bg-linear-to-br from-cyan-50 via-white to-amber-50 p-5 shadow-sm md:p-6">
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<h1 className="mt-3 text-2xl font-bold text-slate-800">Data Atur Lokasi</h1>
							<p className="text-sm text-slate-600">Kelola penempatan item di lokasi vihara.</p>
						</div>
				...
				{isEmpty && (
					<Empty
						...
						desc="Tambahkan penempatan item baru untuk mulai mengelola lokasi vihara."
						action={
							<LinkButton
								href="/admin/assign-item/create"
								size="small"
							...
							>
								<div className="flex items-start justify-between gap-3">
									...
									<div className="flex shrink-0 gap-2 text-sm">
										<Link
											href={`/admin/assign-item/${item.id}`}
